...
uint32_t GroupBulkRead::getBulkData(uint8_t id, uint16_t address, uint16_t data_length, uint8_t* storage)
{
  if (isAvailable(id, address, data_length) == false)
    return 0;

  uint16_t start_addr = address_list_[id];
	storage[0]=data_list_[id][address - start_addr];
	storage[1]=data_list_[id][address - start_addr + 1];
	storage[2]=data_list_[id][address - start_addr + 2];
	storage[3]=data_list_[id][address - start_addr + 3];
	
	storage[4]=data_list_[id][address - start_addr + 4];
	storage[5]=data_list_[id][address - start_addr + 5];
	storage[6]=data_list_[id][address - start_addr + 6];
	storage[7]=data_list_[id][address - start_addr + 7];

	storage[8]=data_list_[id][address - start_addr + 8];
	storage[9]=data_list_[id][address - start_addr + 9];
	storage[10]=data_list_[id][address - start_addr + 10];
	storage[11]=data_list_[id][address - start_addr + 11];
